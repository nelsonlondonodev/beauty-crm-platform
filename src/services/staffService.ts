...
export const crearEmpleado = async (
  empleado: Omit<Empleado, 'id' | 'created_at' | 'activo'> & {
    activo?: boolean;
  }
) => {
  const { data, error } = await supabase
...
export const getPendientesEmpleados = async () => {
  ...
  const result = empleados.map((emp) => {
    const empComisiones =
      comisiones?.filter((c) => c.empleado_id === emp.id) || [];
    const totalGanado = empComisiones.reduce(
      (sum, item) => sum + (Number(item.comision_monto) || 0),
      0
    );

    const empPagos = pagos?.filter((p) => p.empleado_id === emp.id) || [];
    const totalPagado = empPagos.reduce(
...
export const getEmpleadosConSaldos = async () => {
  ...
  return empleados.map((emp) => {
    ...
    const totalVentas = empItems.reduce(
      (sum, item) => sum + (Number(item.precio_total) || 0),
      0
    );
    const totalGanado = empItems.reduce(
      (sum, item) => sum + (Number(item.comision_monto) || 0),
      0
    );
